...
export async function runDoctorCommand(argv: string[]): Promise<void> {
  ...
  if (await fileExists(path.join(target, "package.json"))) {
    const pkg = await readJsonFile<{ name?: string }>(path.join(target, "package.json"));
    console.log(yellow(`Repo package name: ${pkg.name ?? "unknown"}`));
  }

  if (!config.androidProjectPath) {
    console.log(
      yellow("No supported Android project detected. Expected android/ or apps/mobile/android."),
    );
  }
}
