...
export async function runInitCommand(argv) {
    ...
    if (!config.androidProjectPath) {
        console.log(yellow("No android project detected. Proceeding with generic templates only."));
    }
    ...
    await writeFileSafe(path.join(config.infraDir, "README.md"), renderReadme({
        androidProjectPath,
        runnerLabel,
        androidApplicationId,
        keystorePath,
